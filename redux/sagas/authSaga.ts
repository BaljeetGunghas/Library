...
function* handleLogin(action: PayloadAction<LoginForm>) {
  try {
    ...
    const { jsonResponse, output, message } = response;

    if (output === 1 && jsonResponse) {
      yield put(loginSuccess({ user: jsonResponse, token: jsonResponse.token }));
...
function* handleRegister(
  ...
  try {
    ...
    if (output === 1 && jsonResponse) {
      yield put(registerSuccess({ user: jsonResponse, token : token || '' }));
